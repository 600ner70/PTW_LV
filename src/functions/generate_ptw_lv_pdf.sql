create or replace FUNCTION         generate_ptw_lv_pdf (
    ...
    CURSOR c_isolation IS
        SELECT * FROM ptw_pro.ptw_lv_equipment_isolation
        WHERE permit_id = p_permit_id
        ORDER BY row_number;

    -- 1:many -- one page 2 printed per monitoring record
...
BEGIN
    ...
    BEGIN
        SELECT * INTO v_permit FROM ptw_pro.ptw_lv_permits WHERE permit_id = p_permit_id;
    EXCEPTION
    ...
    BEGIN
        SELECT * INTO v_cm FROM ptw_pro.ptw_lv_control_measures WHERE permit_id = p_permit_id;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN NULL;
    END;

    -- Count monitoring records so footer knows whether page 2 was printed
...
<style>
    ...
    .header-refs {
        ...
        border-top: 1px solid rgba(255,255,255,0.3); padding-top: 6px; margin-top: 4px;
    }
...
<div class="ptw-lv-report-container">';
    ...
    <div class="ptw-lv-header">
        ...
        <div class="header-refs">
            <div>Safety Programme Reference No.:<br><strong>' || safe_val(v_permit.safety_programme_ref_no) || '</strong></div>
            <div>Isolation &amp; Earthing Diagram Serial No.:<br><strong>' || safe_val(v_permit.isolation_diagram_serial_no) || '</strong></div>
            <div>Permit to Work No.:<br><strong>' || safe_val(v_permit.permit_number) || '</strong></div>
        </div>
    </div>';
    ...
    -- Text matches physical form wording
    -- ========================================
    v_html := v_html || '
    <div class="ptw-lv-section">
    ...
    <div class="ptw-lv-section">
        ...
        <div style="font-size:7.5pt;color:#555;padding:4px 12px;font-style:italic;border-bottom:1px solid #eee;">
    ...
    <div class="ptw-lv-section">
        ...
        <div class="declaration-box">
            I have reviewed all aspects of the task/activity and I am satisfied with the arrangements
            as detailed within the relevant risk assessment and method statement that have been put in
            place and certify that this activity detailed is authorised to proceed.
        </div>
        <div class="ptw-lv-row">
    ...
    <div class="ptw-lv-section">
        ...
        <div class="declaration-box">
            I certify that I am competent to supervise and undertake the works detailed within this
            Permit to Work and have read and fully understand the documentation associated with this
            work activity.
        </div>
        <div class="ptw-lv-row">
    ...
    <div class="ptw-lv-section">
        ...
        <div class="declaration-box">
            ...
            The area [has]/[has not] been left in a safe and tidy condition and all waste has been removed from site.
            It is <strong>' || get_safe_label(v_permit.clear_area_safe) || '</strong>
    ...
    <div class="ptw-lv-section">
        ...
        <div class="declaration-box">
            ...
            and that I have checked that the place of work has been left in a safe and tidy condition.
            Where the work is not complete a further Permit may be required.
        </div>
        <div class="ptw-lv-row">
            <div class="ptw-lv-label">Works complete:</div>
    ...
    -- ========================================
    -- PAGE 1 FOOTER (matches physical form)
    -- ========================================
    v_html := v_html || '
    <div class="footer">
        During the works the Original copy shall be retained by the &lsquo;Authorised Person&rsquo; and the copy
        shall be retained by the &lsquo;Person in Charge&rsquo; of the work and must have it available at all times
        for inspection. On completion of the works, the copy must be returned and both parts are signed
        when cancelling this Permit. Retention period 5 years.
    ...
    -- One page 2 printed per monitoring record.
    -- Cursor loops over all records in date order.
    -- ========================================
    ...
    <div class="ptw-lv-section">
        ...
        v_html := v_html || '
                <tr>
                    <td style="vertical-align:top;">
                        <div style="font-size:8pt;font-weight:700;margin-bottom:3px;">Check 1) Detail of check made:</div>
                        <div>' || NVL(safe_val(v_mon.ms_check1_detail), '&nbsp;') || '</div>
                        <div style="margin-top:6px;font-size:8pt;"><strong>Enter time of check:</strong> ' || NVL(v_mon.ms_check1_time, '-') || '</div>
                    </td>
                    <td style="vertical-align:top;">
        ...
        IF v_mon.ms_check2_detail IS NOT NULL THEN
            v_html := v_html || '
                <tr>
                    <td style="vertical-align:top;">
                        <div style="font-size:8pt;font-weight:700;margin-bottom:3px;">Check 2) Detail of check made:</div>
                        <div>' || safe_val(v_mon.ms_check2_detail) || '</div>
                        <div style="margin-top:6px;font-size:8pt;"><strong>Enter time of check:</strong> ' || NVL(v_mon.ms_check2_time, '-') || '</div>
                    </td>
                    <td style="vertical-align:top;">
                        <div><strong>All in order:</strong>&nbsp;&nbsp;' ||
        ...
        IF v_mon.ms_check3_detail IS NOT NULL THEN
            v_html := v_html || '
                <tr>
                    <td style="vertical-align:top;">
                        <div style="font-size:8pt;font-weight:700;margin-bottom:3px;">Check 3) Detail of check made:</div>
                        <div>' || safe_val(v_mon.ms_check3_detail) || '</div>
                        <div style="margin-top:6px;font-size:8pt;"><strong>Enter time of check:</strong> ' || NVL(v_mon.ms_check3_time, '-') || '</div>
                    </td>
                    <td style="vertical-align:top;">
                        <div><strong>All in order:</strong>&nbsp;&nbsp;' ||
    ...
    -- If no monitoring records, just add generated timestamp
