  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE "PTW_PRO"."PTW_PERMIT_SAVE_PKG" AS

    -- Saves (inserts or updates) a permit's Page 2 header
    -- fields. Called identically from Page 2's online
    -- "Save Permit Data" process and its "SYNC_OFFLINE_DATA"
    -- Ajax Callback - same logic, same guards, same site
    -- resolution, regardless of which context calls it.
    --
    -- p_permit_id: IN NULL means INSERT a new permit (OUT
    --   returns the new permit_id). IN NOT NULL means UPDATE
    --   that permit (OUT returns the same value unchanged).
    -- p_ptw_type: on INSERT, defaults to 'LV ISOLATION' if
    --   NULL. On UPDATE, a NULL leaves the permit's existing
    --   type untouched (never silently resets an existing
    --   permit's type).
    -- p_site_id / p_site_new_name: pass exactly one. If
    --   p_site_id is provided, it's used directly (an
    --   existing site was selected). If only
    --   p_site_new_name is provided, the procedure resolves
    --   or creates the site itself (same find-or-create +
    --   race-handling logic in both callers, not duplicated).
    -- p_workflow_status: pass the permit's CURRENT
    --   workflow_status (read by the caller before calling
    --   this procedure). If it is not 'DRAFT', the procedure
    --   does nothing and returns p_saved = FALSE,
    --   p_message = 'Permit is no longer in draft - no
    --   changes saved.' - this is the guard that was
    --   previously online-only.
    --
    -- OUT parameters let each caller build its own response:
    --   online sets page items directly from them; offline
    --   builds its JSON {success, message} response from them.
